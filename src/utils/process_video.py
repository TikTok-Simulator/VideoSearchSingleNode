...
def upscale_video_resolution(
    video_url: str, 
    output_dir_relative: str = "../static/processed_file"
) -> str:
    ...
    output_name = os.path.basename(video_url.split('?')[0])  # Remove query params if any
    ...
    if original_width >= 360 and original_height >= 360:
        print("Video resolution is already sufficient (>= 360x360). No upscaling needed.")
        video.release()
    ...
    if original_width < 360:
        ...
        target_width, target_height = original_width, original_height  # Shouldn't hit this due to prior check
    ...
    if not out.isOpened():
        video.release()
        raise ValueError(f"Failed to open VideoWriter with codec 'mp4v' at {output_path}")
    ...
    print(f"Video upscaled to {target_width}x{target_height} and saved to {output_path}")
    return output_path
